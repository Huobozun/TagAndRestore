...
class searchalljson():
    ...
    def _searchword(label1,zhishi,_text,_label):
    #进行label的计数和定位
            ...
            if(zhishi=='0'):   #用一段查找各个标注来确定位置
                ...
                for j in range(0,len(_text)):
                    if(_text[j]!=None and _text[j]!={0: None} and _text[j]!='' and _text!=[]):
                        [a0,a1]=searchalljson.searchnumber(label1,_text[j],_label)
                        b+=a0
                        #print(j+1,_label,a0,a1)
                        #print(a1)
                        if(a1!=[] and a1!=None and a1!='' and a1!={0: None}):
    ...
    def getresult(label1,text,name_en):
        ...
        if isinstance(text,list):
            for i in range(0,len(text)):
                [n,l]=searchalljson._searchword(label1,'0',text[i],name_en)
                if(n>0):
                    str6=(i,l)
                    localall.append(str6)
        else:
            [n,l]=searchalljson._searchword(label1,'0',text,name_en)
            if(n>0):
                str6=(i,l)
                localall.append(str6)
        ...
        if isinstance(name_en,list):
            for i in range(0,len(name_en)):
                [n,l]=searchalljson._searchword(label1,'1',text,name_en[i])
                if(n>0):
                    nlabel+=1
                num+=n
        else:
            [n,l]=searchalljson._searchword(label1,'1',text,name_en)
            if(n>0):
                    nlabel+=1
            num+=n
        #print(localall)
    ...
    def _search(self):
        ...
        he=[]
        henum=[]
        with open(self.label0+self.label1+'2.tsv','w',newline='')as f:
            for i in range(0,len(self.filelist)):
                    ...
                    if isinstance(self.label,dict):
                        ...
                        for j in range(0,nlabel):
                            ...
                            if isinstance(llabel,str):
                                ...
                                xllabel=llabel
                            if(j==0):
                                [x,x2,xl3]=searchalljson.getresult(self.label1,self.text,xllabel)
                            else:
                                [x,x2,xl33]=searchalljson.getresult(self.label1,self.text,xllabel)
                                for jj in range(0,len(xl33)):
                                    xl3.append(xl33[jj])
                            if(x>0):
                                bnum[i]+=1
                        ...
                        [x,x2,xl3]=searchalljson.getresult(self.label1,self.text,self.label)
                ...
                #除去indications的其他标注按照每出现包含的每个单词记为可匹配一次
                    ...
                    bnum.append(0)
                    [x,x2,xl3]=searchalljson.getresult(self.label1,self.text,self.label)#x是每次可还原标注在文中匹配量，x2是可还原标注量，xl3是[（段落，[（位置，'匹配词'）])]
...
[aa1,anum1,ahe1,ahenum1]=a1._search()
[aa2,anum2,ahe2,ahenum2]=a2._search()
[aa3,anum3,ahe3,ahenum3]=a3._search()
[aa4,anum4,ahe4,ahenum4]=a4._search()
...
with open('indications2.tsv','w',newline='')as f:
    xhe=[]
    for i in range(0,len(ahenum1)):
        xhe.append(ahe1[i]+ahe2[i]+ahe3[i]+ahe4[i])
        #print(ahenum1[i])
        f.write('%s\t%s\n' %(ahenum1[i],json.dumps(xhe[i])))
